...
initial algorithm
 ...
 motDer :=
   ...
   elseif ( size(motorEfficiency.r_V, 1) == 1)  then
      ...
      ensureMonotonicity=IDEAS.Utilities.Math.Functions.isMonotonic(x=motorEfficiency.eta,
                                                                        strict=false));
  hydDer :=
     if use_powerCharacteristic then
...
//  WFlo = eta * P;
  ...
  if homotopyInitialization then
    Q_flow = homotopy(actual=IDEAS.Utilities.Math.Functions.spliceFunction(pos=QThe_flow, neg=0,
                       x=noEvent(abs(VMachine_flow))-2*delta_V_flow, deltax=delta_V_flow),
                     simplified=0);
  else
    Q_flow = IDEAS.Utilities.Math.Functions.spliceFunction(pos=QThe_flow, neg=0,
                       x=noEvent(abs(VMachine_flow))-2*delta_V_flow, deltax=delta_V_flow);
  end if;
  annotation (
